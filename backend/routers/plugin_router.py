    ...
    def register_endpoints(self) -> None:
        ...
        def get_plugin_detail(plugin_name: str, _=VerifiedDep) -> PluginDetailResponse:
            ...
            try:
                ...
                if not plugin_info:
                    logger.error(f"[PluginRouter] 无法获取插件信息: {plugin_name}")
                    return PluginDetailResponse(
                        success=False,
                ...
                logger.info(f"[PluginRouter] 插件配置路径: {config_path}")
                
                plugin_enabled = plugin_instance.enable_plugin if plugin_instance else False
                logger.info(f"[PluginRouter] 插件启用状态: {plugin_enabled}")
                ...
                logger.info(f"[PluginRouter] 返回插件详情: components={len(components)}, enabled={plugin_enabled}")
        ...
        async def enable_component(
            ...
            """启用指定的组件"""
            try:
                logger.info(f"[PluginRouter] 启用组件请求: plugin={plugin_name}, component={component_name}, type={component_type}")
                logger.info(f"[PluginRouter] ComponentType 可用值: {[e.name for e in ComponentType]}")
                
                # 转换组件类型
                try:
                    comp_type = ComponentType[component_type.upper()]
                    logger.debug(f"[PluginRouter] 组件类型转换: {component_type} -> {comp_type}")
                except KeyError:
                    available_types = [t.name for t in ComponentType]
                    logger.error(f"[PluginRouter] 无效的组件类型: {component_type}, 可用类型: {available_types}")
                    return OperationResponse(
                        success=False,
                        error=f"无效的组件类型: {component_type}，可用类型: {', '.join(available_types)}"
                    )
                
                success = await component_state_api.set_component_enabled(component_name, comp_type, True)
                logger.info(f"[PluginRouter] 启用组件结果: {success}")
                
                if success:
                    return OperationResponse(
                        success=True,
                        message=f"组件 '{component_name}' 已启用"
                    )
                else:
                    return OperationResponse(
                        success=False,
                        error=f"启用组件 '{component_name}' 失败"
                    )
            except Exception as e:
        ...
        async def disable_component(
            ...
            """禁用指定的组件"""
            try:
                logger.info(f"[PluginRouter] 禁用组件请求: plugin={plugin_name}, component={component_name}, type={component_type}")
                logger.info(f"[PluginRouter] ComponentType 可用值: {[t.name for t in ComponentType]}")
                ...
                logger.info(f"[PluginRouter] 禁用组件结果: {success}")
                
                if success:
                    return OperationResponse(
                        success=True,
                        message=f"组件 '{component_name}' 已禁用"
                    )
                else:
                    return OperationResponse(
                        success=False,
                        error=f"禁用组件 '{component_name}' 失败"
                    )
            except Exception as e:
